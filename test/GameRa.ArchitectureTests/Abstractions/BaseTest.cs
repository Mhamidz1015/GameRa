namespace GameRa.ArchitectureTests.Abstractions;

public abstract class BaseTest
{
    protected const string UsersNamespace = "GameRa.Modules.Users";
    protected const string UsersIntegrationEventsNamespace = "GameRa.Modules.Users.IntegrationEvents";

    protected const string GamesNamespace = "GameRa.Modules.Games";
    protected const string GamesIntegrationEventsNamespace = "GameRa.Modules.Games.IntegrationEvents";

    protected const string StoreNamespace = "GameRa.Modules.Store";
    protected const string StoreIntegrationEventsNamespace = "GameRa.Modules.Store.IntegrationEvents";

    protected const string LibraryNamespace = "GameRa.Modules.Library";
    protected const string LibraryIntegrationEventsNamespace = "GameRa.Modules.Library.IntegrationEvents";
}
