...
internal sealed class GetDiscountQueryHandler(IDbConnectionFactory dbConnectionFactory)
    : IQueryHandler<GetDiscountQuery, DiscountResponse>
{
    public async Task<Result<DiscountResponse>> Handle(GetDiscountQuery request, CancellationToken cancellationToken)
    {
        ...
        DiscountResponse? discount = await connection.QuerySingleOrDefaultAsync<DiscountResponse>(sql, request);

        if (discount is null)
        {
            return Result.Failure<DiscountResponse>(DiscountErrors.NotFound(request.DiscountId));
        }
