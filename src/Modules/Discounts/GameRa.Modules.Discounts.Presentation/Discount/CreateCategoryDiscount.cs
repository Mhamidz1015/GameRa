...
internal sealed class CreateCategoryDiscount : IEndpoint
{
    public void MapEndpoint(IEndpointRouteBuilder app)
    {
        app.MapPost("discounts/category", async (Request request, ISender sender) =>
        {
            var command = new CreateCategoryDiscountCommand(
                request.Code,
                request.Type,
                request.Amount,
                request.CategoryId,
